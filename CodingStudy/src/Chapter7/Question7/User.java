...
public class User {
	...
	private HashMap<Integer, AddRequest> receiveAddRequests = new HashMap<Integer,AddRequest>();
	...
	public void receiveAddRequest(AddRequest request) {
		int senderId = request.getFromUser().getId();
		if(!receiveAddRequests.containsKey(senderId)) {
			receiveAddRequests.put(senderId, request);
		}
	}
	...
	public void removeAddRequest(AddRequest request) {
		if(request.getToUser() == this) {
			receiveAddRequests.remove(request);
		}else if(request.getFromUser() == this) {
			sentAddRequests.remove(request);
		}
	}
